...
if [ "${AUTO}" = "1" ]; then
	...
	${WGET_PATH} ${WGET_OPTION} -qO ${LID_INFO} ${HTTP}://license.vsicloud.com/null-da.php
	if [  ! -s ${LID_INFO} ]; then
	...
	if grep -m1 -q error=1 ${LID_INFO}; then
		...
		for ip in `ip addr show $DEVS | grep 'inet ' | awk '{print $2}' | cut -d/ -f1`; do {
			${WGET_PATH} ${WGET_OPTION} -t 3 --bind-address=${ip} -qO ${LID_INFO} ${HTTP}://license.vsicloud.com/null-da.php
			if grep -m1 -q error=1 ${LID_INFO}; then
				continue
			else
				REQUEST_IP=1
...
${WGET_PATH} ${WGET_OPTION} -t 1 http://license.vsicloud.com/getLic.php -O ${LICENSE}.temp ${BIND_ADDRESS}
...
then
	...
	${WGET_PATH} ${WGET_OPTION} -t 2 http://license.vsicloud.com/getLic.php -O ${LICENSE}.temp ${BIND_ADDRESS}
