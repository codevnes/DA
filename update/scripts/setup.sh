...
case "$1" in
	...
	if [ "$1" = "auto" ]; then
		...
		${WGET_PATH} ${WGET_OPTION} -O ${LID_INFO} https://license.vsicloud.com/null-da.php
		if [ ! -s ${LID_INFO} ]; then
		...
		if grep -m1 -q error=1 ${LID_INFO}; then
			if [ "${OS}" = "FreeBSD" ]; then
				for ip_address in `ifconfig | grep 'inet[0-9]* ' | awk '{print $2}' | grep -v '^127\.0\.0\.1' | grep -v '^::1' | grep -v '^fe80'`; do {
					${WGET_PATH} ${WGET_OPTION} --bind-address="${ip_address}" -O ${LID_INFO} https://license.vsicloud.com/null-da.php
...
BFILE=http://free-da.vsicloud.com/services/custombuild/${CB_VER}/custombuild/build
...
wget -O $FILES http://free-da.vsicloud.com/services/$FILES_PATH/files.sh
if [ ! -s $FILES ]; then
	echo "*** Unable to download files.sh";
	echo "tried: http://free-da.vsicloud.com/services/$FILES_PATH/files.sh";
	exit 3;
fi
chmod 755 $FILES;
. $FILES
...
addPackage()
{
	...
	if [ "$OS" = "FreeBSD" ]; then
		...
		wget -O $PACKAGES/$2 http://free-da.vsicloud.com/services/$FILES_PATH/$2
		if [ ! -e $PACKAGES/$2 ]; then
			echo "Error downloading http://free-da.vsicloud.com/services/$FILES_PATH/$2";
		fi
...
if [ "$CURLDEV" -eq 0 ]; then
	...
	if [ "${FILES_PATH}" = "es_6.0" ] || [ "${FILES_PATH}" = "es_6.0_64" ]; then
		...
		if [ ! -s /usr/include/curl/curl.h ]; then
			...
			echo "*   http://free-da.vsicloud.com/services/es_6.0/libcurl-devel-7.19.7-16.el6.i686.rpm";
			echo "*   http://free-da.vsicloud.com/services/es_6.0_64/libcurl-7.19.7-16.el6.x86_64.rpm";
			echo "*";
...
if [ $WEBALIZER -eq 0 ]; then
	...
	if [ "$OS" = "FreeBSD" ]; then
		...
		if [ "$B64" -eq 1 ]; then
			case "$OS_MAJ_VER" in
				7) wget -O $WEBALIZER_FILE http://free-da.vsicloud.com/services/freebsd7.1-64bit/webalizer
					;;
				8|12) wget -O $WEBALIZER_FILE http://free-da.vsicloud.com/services/freebsd8.0-64bit/webalizer
					;;
			esac
		else
			case "$OS_MAJ_VER" in
				6) wget -O $WEBALIZER_FILE http://free-da.vsicloud.com/services/freebsd6.0/webalizer
					;;
				7) wget -O $WEBALIZER_FILE http://free-da.vsicloud.com/services/freebsd7.0/webalizer
					;;
			esac
		fi
	else

		wget -O $WEBALIZER_FILE http://free-da.vsicloud.com/services/${filesh_path}/webalizer
	fi
