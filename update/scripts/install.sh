...
OS=`uname`
CBVERSION="2.0"
DL_SERVER=free-da.vsicloud.com
BACKUP_DL_SERVER=free-da.vsicloud.com
...
getServices() {
	...
	if [ ! -s ${SERVICES_FILE}.md5 ]; then
		...
		if [ -s ${SERVICES_FILE}.md5 ]; then
			...
			${WGET_PATH} ${WGET_OPTION} http://free-da.vsicloud.com/services/${servfile}.md5 -O ${SERVICES_FILE}.md5
			if [ -s ${SERVICES_FILE}.md5 ]; then
				echo "************************"
				echo "Downloading from ${DL_SERVER} and ${BACKUP_DL_SERVER} failed.  Switching to the functional free-da.vsicloud.com"
				echo "************************"
				DL_SERVER=free-da.vsicloud.com
			fi
		fi
